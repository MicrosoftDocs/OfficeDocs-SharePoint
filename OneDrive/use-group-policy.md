...
The B2B Sync feature of the OneDrive sync client allows users at an organization to sync OneDrive and SharePoint libraries and folders shared with them from another organization. [Learn about OneDrive B2B Sync](b2b-sync.md).
...
If you enable this setting, new users who set up the sync client won't download online-only files by default. The files don’t download to the device until they are opened. If you disable this setting, Windows 10 users will have the same sync behavior as users of previous versions of Windows, and won't be able to turn on Files On-Demand. If you do not configure this setting, users can turn Files On-Demand on or off.
