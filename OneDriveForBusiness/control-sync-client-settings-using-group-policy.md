...
- [Allow syncing OneDrive accounts for only specific organizations](control-sync-client-settings-using-group-policy.md#TenantAllowList)
    
- [Block syncing OneDrive accounts for specific organizations](control-sync-client-settings-using-group-policy.md#TenantBlockList)
    
- [Enable OneDrive Files On-Demand](control-sync-client-settings-using-group-policy.md#FilesOnDemand)
    
- [Migrate pre-existing team sites with OneDrive Files On-Demand](control-sync-client-settings-using-group-policy.md#TeamSiteFOD)
    
- [Prevent OneDrive from generating network traffic until the user signs in to OneDrive](control-sync-client-settings-using-group-policy.md#PreventNetworkTraffic)
    
- [Set the maximum percentage of upload bandwidth that OneDrive.exe uses](control-sync-client-settings-using-group-policy.md#MaxBandwidth)
    
- [Silently configure OneDrive using Windows 10 or domain credentials](control-sync-client-settings-using-group-policy.md#SilentConfig)
    
- [(Preview) Configure the maximum OneDrive size for downloading all files automatically](control-sync-client-settings-using-group-policy.md#MaxOneDriveSize)
...
- [Set the default location for the OneDrive folder](control-sync-client-settings-using-group-policy.md#DefaultRootDir)
    
- [Prevent users from changing the location of their OneDrive folder](control-sync-client-settings-using-group-policy.md#DisableCustomRoot)
    
- [Prevent users from synchronizing personal OneDrive accounts](control-sync-client-settings-using-group-policy.md#DisablePersonalSync)
    
- [Set the maximum download throughput that OneDrive.exe uses](control-sync-client-settings-using-group-policy.md#SetMaxDownloadTput)
    
- [Coauthoring and in-app sharing for Office files](control-sync-client-settings-using-group-policy.md#EnableAllOcsiClients)
    
- [Delay updating OneDrive.exe until the second release wave](control-sync-client-settings-using-group-policy.md#EnableEnterpriseUpdate)
    
- [Users can choose how to handle Office files in conflict](control-sync-client-settings-using-group-policy.md#EnableHoldTheFile)
    
- [Prevent users from using the remote file fetch feature to access files on the computer](control-sync-client-settings-using-group-policy.md#RemoteAccessGPOEnabled)
    
- [Set the maximum upload throughput that OneDrive.exe uses](control-sync-client-settings-using-group-policy.md#SetMaxUploadTput)
...
For more info about using this policy as part of redirecting Windows known folders (such as the Documents folder) to OneDrive, see [Redirect known folders to OneDrive for Business](redirect-known-folders-in-windows).
