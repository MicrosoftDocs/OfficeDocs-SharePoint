...
- If you need to change your font asset library, you will need to first [remove the font asset library](/powershell/module/sharepoint-online/remove-spoorgassetslibrary?) and then use the ```Remove-SPOOrgOrgAssetsLibrary``` command. Repeat the process above to upload your updates in the font files. Changes made to the Document Library on the SharePoint web platform may affect font availability and feature functionality.
