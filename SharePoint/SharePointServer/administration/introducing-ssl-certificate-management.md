...
|KeyAlgorithm (ECC / RSA) | Specifies the key algorithm for your certificate. This choice will be used for both the public key and the private key of your certificate. <p>`RSA`is the most common and widely supported key algorithm for certificates. Select the `RSA` algorithm if you're unsure, which type of key your certificate authority supports.<p>`ECC` uses elliptic curve cryptography based on ECDSA keys with`NIST P-256`, `P-384`, or `P-521` curves. SSL/TLS connections are faster to complete with `ECC`certificates than `RSA` certificates at the equivalent security strength. Verify that your certificate authority supports ECC certificates before selecting it. If this parameter isn't specified, the default key algorithm of the farm will be used.|
...
```powershell
$password = ConvertTo-SecureString -AsPlainText -Force Export-SPCertificate -Identity "Contoso SharePoint (2020)" -Password $password -IncludeAllCertificatesInCertificationPath -IncludeExtendedProperties -Path "\server\fileshare\certificates.pfx"
```
