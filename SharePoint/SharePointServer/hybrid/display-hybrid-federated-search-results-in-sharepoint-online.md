...
The search results from SharePoint Server appear with the search results from SharePoint Server, but in a separate group called a result block. You can configure the block of results from SharePoint Server to appear above all the results from SharePoint Online, or to be ranked by relevance compared to the SharePoint Online results.
...
In this procedure, you create a query rule in SharePoint Online that uses the result source that you created in the previous procedure in this article. When the query rule fires, it causes search results from content in the SharePoint Server search index to appear in a result block on a search results page in SharePoint Online.
...
![Image of hybrid search results in SharePoint Online](../media/HybridSearchResultsInSPO.jpg)
