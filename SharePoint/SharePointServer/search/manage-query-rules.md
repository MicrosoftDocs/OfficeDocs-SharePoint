...
Without using any custom code, a Search service application administrator, site collection administrator, or site owner can improve search results by creating and managing query rules. Query rules help searches respond to the intent of users. 
...
> [!IMPORTANT]
> Query rules only affect **modern** search when the action in the rule is to promote an individual result towards the top of search results, the rule is defined for the default result source, and the rule is defined at the Search service application level. Users can only get promoted results when they’ve searched for results across all of SharePoint and they only see promoted results on the search results page in the filtered view **All result types** (default view). 
