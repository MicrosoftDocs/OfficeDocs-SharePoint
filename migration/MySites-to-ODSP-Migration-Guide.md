...
Before migrating your SharePoint My Sites content, you must pre-provision your users in OneDrive: 
...
|**Scan**|**Description**|
|:-----|:-----|
|[File Versions](https://docs.microsoft.com/en-us/sharepointmigration/migration-assessment-scan-file-versions) |**Why:** The more versions of a file you have| the longer it will take to migrate. Note:  By default| versioning is enabled for all lists and libraries on the target platform. In the destination SPO site| there is no limit when versioning is enabled.</br></br>Action:  Consider limiting the number of versions you migrate.|
|[Large Lists](https://docs.microsoft.com/en-us/sharepointmigration/migration-assessment-scan-large-lists |**Why:** Lists over 20,000 items have caused issues with migration| making it more difficult to predict how long it takes to migrate sites. List data is migrated| but the larger the list the more unpredictable the migration process has proven. Extremely large lists can result in an extended migration.|
|[Long OneDrive URLs](https://docs.microsoft.com/en-us/sharepointmigration/migration-assessment-scan-long-onedrive-urls) |**Why:** Content with long URLs exceeding the limit will be skipped.  They will not migrate. Action:  Look at |
|[Checked out files](https://docs.microsoft.com/en-us/sharepointmigration/migration-assessment-scan-checked-out-files) |**Why:**  Only checked-in content will be migrated. Action: Have users check in their files prior to migration to avoid data loss.|
|[Large Excel Files](https://docs.microsoft.com/en-us/sharepointmigration/migration-assessment-scan-large-excel-files) |**Why:** If you attempt to open a file larger than 10MB  from OneDrive (online)| it will prompt you to open the file in the Excel client application."|
|[Large List Views](https://docs.microsoft.com/en-us/sharepointmigration/migration-assessment-scan-large-list-views)|**Why:** In your My Site| you can configure list view throttling so there are a set number of hours per day where the throttle on views is lifted. In OneDrive| the limit is in place continuously (24x7). While your lists and data will be migrated| some of your list views be throttled. "|
|[Browser File Handling](https://docs.microsoft.com/en-us/sharepointmigration/migration-assessment-scan-browser-file-handling)|**Why:** SharePoint Server allows you to set it from “strict” to “permissive”.  However| in SharePoint Online and OneDrive| the “strict” setting is enforced and can’t be modified.  Data will be migrated| but the behavior with the HTM and HTML files will change from opening within the browser to prompting the user to download. "|
|[InfoPath](https://docs.microsoft.com/en-us/sharepointmigration/migration-assessment-scan-infopath) |**Why:**  InfoPath lets developers build custom forms for accepting user input in a variety of locations throughout SharePoint. However| certain features of custom InfoPath forms will not be migrated. "|
...
Use the SharePoint Migration Tool (SPMT) as an easy to use option for migrating your existing My Sites to OneDrive. SPMT is a free tool that lets you migrate lists or files from your on-premises file shares or from SharePoint Server 2013 to either SharePoint or OneDrive. 
...
- [Collaborate in OneDrive in OneDrive](https://support.office.com/en-us/article/collaborate-in-onedrive-d8a2a19a-e306-4ca5-9b00-19b0e96890d6?ui=en-US&rs=en-US&ad=US)
