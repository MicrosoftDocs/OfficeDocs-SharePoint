...
|Do not migrate files with these extensions|Enter a list of of file extensions of the types of files you do not want to migrate. Separate each extension entered with a colon and do not include the dot (.txt would be entered as txt). </br> **Note** For files with multiple file extensions, eg. .ext1.ext2, add only the last extension, .ext2, to the exclusion list.|
