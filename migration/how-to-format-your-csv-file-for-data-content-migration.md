...
SPMT lets you use a comma-separated value (CSV) file to bulk migrate your data. Use any text editor, or an application like Excel, to create the CSV file.
  
 **CSV file format**
  
You CSV column needs six columns. The first three are source values that provide detail about where your data is currently located. The remaining three columns indicate the site, document library, and optional subfolder where you are migrating your data to. All six columns must be accounted for in the file, even if you don't need a value for a given field.
...
The following example shows how it would appear in a .txt file.
...
The following example uses Microsoft Excel to create the CSV file.
...
2. Enter the values for your migration jobs. Enter one migration source and destination per row. See the following reference table for further explanation of columns.
  ...
  - **Column B:** Enter name of the SharePoint Server document library that you're migrating. If you leave this field empty, all document libraries will be migrated. If you're migrating a file share, leave this column empty. *Optional.* 
...
|Source DocLib | *Optional*. Enter name of the SharePoint Server document library that you're migrating. If you leave this field empty, all document libraries will be migrated. If you're migrating a file share, leave this column empty. |
...
|Target Web | *Required*. Enter the SharePoint site URL where the files are to be migrated to. |
|Target DocLib | *Required*. Enter the name of the document library with the SharePoint site where the files are to be migrated to. |
...
The following example shows the JSON format used to migrate your data.
