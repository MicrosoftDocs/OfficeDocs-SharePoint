...
description: "How to Migrate from Dropbox to Office 365"
---
...
Undertaking any corporate migration is a daunting task. When moving from Dropbox to Office 365, there are many things to consider. Our team has done this enough times to have learned a thing or two. We hope this guide helps you migrate successfully from Dropbox to Office 365.

If you experience any issues, do not hesitate to ask us questions!

### File Processing Summary

When we transfer a file, a temporary copy is downloaded from **Dropbox** to a temporary server and then uploaded to Office 365. Upon successful upload, that file is deleted from the temporary server. When your migration is complete, that temporary server is eliminated. Any log data expires in 90 days and is never retained by us. We do not perform any actions beyond copying files and folders and sharing permissions. We never perform delete operations.
...
- What are the differences between Dropbox and Office 365, and how do I reconcile them?
...
This also applies for separate **Dropbox Team Folders** migrating to a single destination user account.

For example:

- /DropboxTeam01 => user01@example.com
- /DropboxTeam02 => user01@example.com

This qualifies as two separate *User Licenses* because there are two separate sources going to the same destination.
...
**Collaborators-only**: Your fiscal budget need only include users that own data in Dropbox. Anyone that strictly engages as a collaborator in Dropbox (for example, they only share other users' content), do not have any data copied, and therefore do not incur a fee.

**Setting permissions is free.** Users that strictly collaborate in Dropbox receive equivalent sharing permissions in Office 365 for free.

### Data Ownership

You must consider data ownership. If a user has only shared data, and absolutely no owned data, we transfer no files from Dropbox to Office 365 for that particular individual.
...
When migrating from Dropbox to Office 365, you need to ensure all your users are active and accessible. Knowing exactly when your Dropbox may shut down or expire is key to safely planning enough time to migrate.
...
**Subject**: ATTENTION: Decision to Migrate from Dropbox to Office 365

**Message**: A few months ago, management decided we will leaving Dropbox and transition to Office 365. In Office 365, all employees will have access to cloud storage and its included apps.
...
**Message**: As you know from prior emails, we are moving from Dropbox to Office 365 as our cloud storage provider.

To assist in this migration, we ask all employees to finish working and upload any last changes to files in Dropbox by 17:00 PT on Friday, April 7, 2020. Changes to files or data in Dropbox after this time will not be moved.
...
**Name changes**: If the name of a file or folder changes in Dropbox, we will treat it as a brand new object. This can lead to duplicate files being migrated to Office 365, or worse: entire folders worth of data being duplicated from the changed folder downwards.
...
When moving from Dropbox to Office 365, user roles *on folders* change.
...
![old Dropbox role to new o365 role](media/old-Dropbox-role.png)
...
We do not preserve file versions. During a migration, only the most recent version of a file will be transfered from Dropbox to Office 365.
...
Dropbox restricts sharing based on files or folders already shared. For example, if you have shared a subfolder, Dropbox does not allow you to change the sharing permissions of the individual subfolders or files within that folder, nor will you be allowed to modify the sharing scheme of the parent until you have unshared the subfolder.
...
The original timestamps from Dropbox are preserved when migrating into Office 365.
...
When migrating from Dropbox into Office 365, the *modified by* author is preserved. However, the *created by* is changed to the end user.
...
![authorize drobox connector](media/auth-dropbox-connector.png)</br>
...
Confirm that the users in the Dropbox source match the users in the Office 365 destination. Usually the emails/usernames will match up, but it depends how you structure and name your users. *Be diligent during this step!*
...
You may either auto-discover or upload a permission map file. We automatically pair perfect matches. If a user or group in Dropbox does not have a perfect match in Office 365, you can correct it in our interface.
...
All transfers take advantage of our incremental feature and will only transfer new or modified data differences between Dropbox and Office 365.
...
**Technical clarification**: We compare what you have in Dropbox to what is in Office 365, and we only transfer anything that doesn't already exist, or has a newer timestamp.
...
**Message**: 
Over the weekend, we migrated our team from Dropbox to Office 365.
...
From this point forward, log into your Office 365 account rather than your old Dropbox account.
