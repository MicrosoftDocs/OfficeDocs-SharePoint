---
title: Migrating from Dropbox to Microsoft 365
ms.author: jhendr
...
description: "How to Migrate from Dropbox to Microsoft 365"
---

# How to migrate from Dropbox to Microsoft 365
...
Undertaking any corporate migration is a daunting task. When moving from Dropbox to Microsoft 365, there are many things to consider. Our team has done this enough times to have learned a thing or two. We hope this guide helps you migrate successfully from Dropbox to Microsoft 365.
...
- What are the differences between Dropbox and Microsoft 365, and how do I reconcile them?
...
**Setting permissions is free**. Users that strictly collaborate in Dropbox receive equivalent sharing permissions in Microsoft 365 for free.

### Data ownership

You must consider data ownership. If a user has only shared data, and absolutely no owned data, we transfer no files from Dropbox to Microsoft 365 for that particular individual.
...
When migrating from Dropbox to Microsoft 365, you need to ensure all your users are active and accessible. Knowing exactly when your Dropbox may shut down or expire is key to safely planning enough time to migrate.
...
**Subject**: ATTENTION: Decision to Migrate from Dropbox to Microsoft 365

**Message**: A few months ago, management decided we will leaving Dropbox and transition to Microsoft 365. In Microsoft 365, all employees will have access to cloud storage and its included apps.
...
**Message**: As you know from prior emails, we are moving from Dropbox to Microsoft 365 as our cloud storage provider.
...
**Name changes**: If the name of a file or folder changes in Dropbox, we treat it as a brand new object. This can lead to duplicate files being migrated to Microsoft 365, or worse: entire folders worth of data being duplicated from the changed folder downwards.
...
When moving from Dropbox to Microsoft 365, user roles *on folders* change.
...
We do not preserve file versions. During a migration, only the most recent version of a file are transfered from Dropbox to Microsoft 365.
...
3. **Global Admin or SharePoint Admin**: A window with an **Authorize** button appears. It prompts you to give your Microsoft 365 Connector a display name <optional>. Select **Authorize**.
...
All transfers take advantage of our incremental feature and only transfer new or modified data differences between Dropbox and Microsoft 365.
...
Alternatively, you can select multiple users, and from the **User Actions** dropdown, you can download their most recent collective logs in a zip file that provides them in both HTML and CSV formats.
...
Our incrementals are delta operations which compare your files in Dropbox to files in Microsoft 365. Using this comparison, we copy anything that is new or has changed. This enables us to keep Microsoft 365 data up to date when the very final cutover of users occurs. These incremental passes are an important part of our process.

**Technical clarification**: We compare what you have in Dropbox to what is in Microsoft 365, and we only transfer anything that doesn't already exist, or has a newer timestamp.
...
**Message**: 
Over the weekend, we migrated our team from Dropbox to Microsoft 365.
...
From this point forward, log into your Microsoft 365 account rather than your old Dropbox account.
