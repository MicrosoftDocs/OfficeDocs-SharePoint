...
|Error: The *Scan Status* shows "Unable to access Google" or the report displays either MAUTHACCESSTOKENINVALID or MAUTHREFRESHTOKEN in the result code column.| The cause could be that the failed drives' owners are assigned to organizational units that the Drive service switched off. Action: In the **Google Admin Console** select **Apps > Google Workspace > Service Status**. Change the status of the "Drive and Docs" service to **ON for everyone**.|
