...
No, it doesn't interact with the OneDrive sync app. We recommend disabling the sync app prior to a migration. If it's running during a migration, it tries to sync all the migrating data.
...
1. **Global or SharePoint admin**: Log in to the main Mover app via **app.mover.io**. In the **Transfer Wizard**, select **Authorize New Connector**.
