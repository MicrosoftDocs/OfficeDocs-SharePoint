...
- When authorizing the G Suite Admin source connector, the template showed the prescan warning. As prescan is not needed any more due to its integration in regular scan and transfer process, 
  the warning from the connector authorization template in UI removed.
