...
|**Error**|**Reason**|**Action**|
|:-----|:-----|:-----|
|This navigation node contains an invalid URL: cannot find the object this URL points to. |SharePoint Online will validate the Url before it creates the navigation node. If the Url represents a SharePoint object and the object cannot be found. SPO will reject the node creation query with this error. This can happen if the target of the Url was deleted, moved, not migrated yet, migrated in previous task where the Url mapping cannot be recognized in current job.|Use the same Url mapping between multiple tasks. For example:  </br></br> Task 1:  http://domain/original_path/site -> http://domain/new_path/site </br></br>Task 2: http://domain/original_path/site2 -> http://domain/new_path/site2.  </br></br> Run another task to migrate Navigation nodes pointing to objects that were not migrated previously. Manually created the node in target site.|
|The navigation settings are highly customized. Currently it's not supported. |Global Navigation and/or Current Navigation among the site navigation settings can only be migrated if its value matches a predefined template. Otherwise,the value will be recognized as highly customized and won’t get migrated.|Change the Global Navigation and/or Current Navigation to use one of the predefined templates (Site settings -> Look and Feel -> Navigation). Manually edit the setting on target site.|
|Could not get target Term Store Id. The taxonomy provider of web navigation settings cannot be updated. |The value of Global Navigation and/or Current Navigation of site navigation settings is “Managed Navigation” and the term store scanner was not able to get the target term store id. This can happen if Managed Metadata Services are not properly configured. See documentation on SPMT Managed Metadata migration. |See documentation on SPMT Managed Metadata migration. |
|The taxonomy provider term set Id of web navigation settings is invalid. |The value of Global Navigation and/or Current Navigation of site navigation settings is “Managed Navigation” and the term store scanner was not able to map the term set id. |Make sure the setting on the source site is valid and the term set was successfully migrated.|
|"To update web navigation settings| you need following permission: Add and Customize Pages.|The SPO account used for migration doesn’t have permission to edit site navigation settings. |Grant “Add and Customize Pages” permission to this account. |

## Taxonomy errors
|**Error**|**Reason**|**Action**|
|:-----|:-----|:-----|
|Migrate term store failed due to permission issue. |Current user is not a term store admin. |In the SharePoint Admin Center, open term store setting under classic features, select the term store you want to migrate and set current user as term store admin.|
